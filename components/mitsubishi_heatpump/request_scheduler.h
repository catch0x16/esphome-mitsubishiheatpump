...
namespace devicestate {
    ...
    class IConnection; // forward declaration

    /**
     * @class RequestScheduler
     * @brief Gère la file d'attente des requêtes cycliques et leur enchaînement.
     *
     * Cette classe extrait la logique de gestion des requêtes INFO du composant IConnection
    ...
    public:
        ...
        /**
         * @brief Type de callback pour obtenir le contexte IConnection (pour canSend et onResponse)
         * @return Pointeur vers IConnection (peut être nullptr)
         */
        using ContextCallback = std::function<IConnection* ()>;
         ...
         * @param context_callback Callback pour obtenir le contexte IConnection (pour canSend et onResponse)
         */
        RequestScheduler(
         ...
         * @param context Contexte IConnection pour vérifier canSend (peut être nullptr, utilise context_callback_ si fourni)
         */
        void send_next_after(uint8_t previous_code, IConnection* context = nullptr);
         ...
         * @param context Contexte IConnection pour appeler onResponse (peut être nullptr)
         */
        void mark_response_seen(uint8_t code, IConnection* context = nullptr);

        /**
         * @brief Traite une réponse reçue
         * @param code Le code de la réponse reçue
         * @param context Contexte IConnection pour appeler onResponse (peut être nullptr, utilise context_callback_ si fourni)
         * @return true si la réponse a été traitée, false sinon
         */
        bool process_response(uint8_t code, IConnection* context = nullptr);
    ...
    private:
        ...
        ContextCallback context_callback_;            // Callback pour obtenir le contexte IConnection

        /**
         * @brief Envoie une requête spécifique par son code
         * @param code Le code de la requête à envoyer
         * @param context Contexte IConnection pour vérifier canSend (peut être nullptr)
         */
        void send_request(uint8_t code, IConnection* context = nullptr);
    };
