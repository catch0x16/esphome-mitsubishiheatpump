...
void RequestScheduler::send_request(uint8_t code) {
            ...
            timeout_callback_(tname, req.soft_timeout_ms, [this, code_copy]() {
                ...
                for (auto& r : this->requests_) {
                    if (r.code == code_copy && r.awaiting) {
                        ...
                        this->send_next_after(code_copy);
                        break;
                    }
                }
                });
...
void RequestScheduler::mark_response_seen(uint8_t code) {
    for (auto& req : requests_) {
        if (req.code == code) {
            req.awaiting = false;
            req.failures = 0;
...
void RequestScheduler::send_next_after(uint8_t previous_code) {
...
bool RequestScheduler::process_response(uint8_t code) {
    ...
    mark_response_seen(code);
    send_next_after(code);
    return true;
}
