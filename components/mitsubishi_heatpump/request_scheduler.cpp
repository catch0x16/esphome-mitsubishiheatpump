...
void RequestScheduler::send_request(uint8_t code, IConnection* context) {
...
void RequestScheduler::mark_response_seen(uint8_t code, IConnection* context) {
    // Obtenir le contexte si non fourni mais que le callback est disponible
    if (!context && context_callback_) {
        context = context_callback_();
    }
...
void RequestScheduler::send_next_after(uint8_t previous_code, IConnection* context) {
    // Obtenir le contexte si non fourni mais que le callback est disponible
    if (!context && context_callback_) {
        context = context_callback_();
    }
...
bool RequestScheduler::process_response(uint8_t code, IConnection* context) {
    // Obtenir le contexte si non fourni mais que le callback est disponible
    if (!context && context_callback_) {
        context = context_callback_();
    }
...
void RequestScheduler::loop() {
    // Currently, timeout handling is done via callbacks.
    // This method is intended for future use if needed
    // (for example, to manually check for timeouts in the main loop).
}
